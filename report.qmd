...
In addition to plotting the raw time trends, we summarise convergence quantitatively by computing the average annual change in each indicator for each country. For example, we fit simple linear regressions of each indicator on year and interpret the slope as the average yearly improvement. For the convergence plots, we also fit linear trends to the gaps relative to Japan; a negative slope of the gap means the difference with Japan is shrinking over time.
